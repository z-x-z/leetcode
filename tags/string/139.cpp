...
public:
    ...
    bool wordBreakAnother(string s, vector<string> &wordDict)
    {
        int m = s.size(), n = wordDict.size();
        unordered_set<string> wordSet;
        for (int i = 0; i < n; i++)
    ...
    bool fun(string s, vector<string> &wordDict)
    {
        return wordBreakAnother(s, wordDict);
    }
};
